...
const UBL_NS = 'urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2';
...
/** Referencia a la factura que se ajusta (obligatorio DIAN para ND). */
...
export interface UblDebitNotePayload {
  id: string;
  issueDate: Date;
  issueTime: string;
  currencyCode: string;
...
/**
 * Construye XML UBL 2.1 DebitNote conforme al Anexo Técnico DIAN.
 * Tipo documento 92. Incluye BillingReference a la factura original.
 */
export function buildUblDebitNoteXml(payload: UblDebitNotePayload): string {
  ...
  payload.lines.forEach((line) => {
    const dnLine = root.ele('cac:DebitNoteLine');
    dnLine.ele('cbc:ID').txt(String(line.id)).up();
    dnLine
      .ele('cbc:DebitedQuantity', { unitCode: line.unitCode })
    ...
    dnLine
      ...
      .up();
    dnLine.ele('cac:Item').ele('cbc:Description').txt(line.description).up().up();
    dnLine
      .ele('cac:Price')
      ...
      .up();
    if (line.taxAmount > 0) {
      const tax = dnLine.ele('cac:TaxTotal');
